...
BINUTILS='/usr/local/opt/binutils/bin'
...
# RUBYLOCAL="$HOME/.rbenv/shims"
RUBYGLOBAL="/usr/local/opt/ruby/bin"
...
MARSLOBIN="${iRCHOME}/.marslo/bin"
SCRIPTBIN="${PYTHON3}:${RUBYGLOBAL}:${RUBYGEM}"
GNUPATH="${GNUBINHOME}:${GNUBINH}${GNUWHICH}:${GNUGETOPT}:${GNUGREP}:${GNUTAR}:${GNUFIND}:${GNUINDENT}:${GNUSED}" UTLSPATH="${BINUTILS}:${OPENSSL}:${INETUTILS}:/usr/local/opt/file-formula/bin:/usr/local/opt/gettext/bin"
LDFLAGS="-L/usr/local/opt/binutils/lib -L/usr/local/opt/python@3.9/lib -L/usr/local/opt/sqlite/lib -L$(brew --prefix)/lib"
CPPFLAGS="-I/usr/local/opt/binutils/include -I/usr/local/opt/sqlite/include -I$(brew --prefix)/include"
PKG_CONFIG_PATH="/usr/local/opt/python@3.9/lib/pkgconfig"

ITERM2_ADDITIONAL="${HOME}/.iterm2"

PATH="$PYTHONUSERBASE/bin:$GROOVY_HOME/bin:${SQLITE}:${MACVIM}:${MARSLOBIN}:${SCRIPTBIN}:${GNUPATH}:${HOMEBREW_SBIN}:${UTLSPATH}:${ITERM2_ADDITIONAL}:$PATH"
MANPATH=$GNUMANHOME:$MANPATH
export JAVA_HOME GROOVY_HOME CLASSPATH M2_HOME M2 MAVEN_OPTS PATH MANPATH PKG_CONFIG_PATH LDFLAGS CPPFLAGS PKG_CONFIG_PATH
