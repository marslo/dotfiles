...
#   LastChange : 2025-12-04 02:25:28
...
declare temperature="${MCX_TEMPERATURE:-0.3}"
[[ "${model}" == gpt-5-nano* ]] && temperature="1" # temperature must be 1 for gpt-5-nano
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --                 ) shift; DIFF_OPTS+=("$@"); break ;;
    *                  ) die "unknown option: '$(c 0Mi)$1$(c)'. try $(c 0Gi)-h$(c)/$(c 0Gi)--help$(c)" ;;
  esac
done

"${DO_FZF}" && { type -P fzf >/dev/null 2>&1 || die "--fzf provided but fzf is not installed"; }

function createDiff() {
  local -a excludes=( '**/.ssh/**' '**/*.key' '**/*.pem' '**/id_ed25519*' '**/.netrc' '**/credential.*' )
  local modified=$(git --no-pager diff --color=never "${DIFF_OPTS[@]}" -- . "${excludes[@]/#/:(glob,exclude)}")
...
# more spinner format can be found in https://gist.github.com/marslo/3f4b1eae28902394ad3201d1b5ea5537
function withSpinner() {
  ...
  local hadMonitoring="$(set -o | awk '/monitor/ {print $2}')"  # on/off
  set -m
...
function main() {
  ...
  "${VERBOSE}" && {
    ...
    printf "$(c Wdi) diff size: $(c 0Mi)%d$(c 0Wdi) bytes ($(c 0Mi)~%d$(c 0Wdi) KiB, $(c 0Mi)~%d$(c 0Wdi) MiB)$(c)\n" "${bytes}" "${kib}" "${mib}" >&2;
  }
  ...
  "${VERBOSE}" && {
    printf "$(c Wdi) payload dumped to \`$(c 0Mi)%s$(c 0Wdi)\` (size: $(c 0Mi)%d$(c 0Wdi) bytes, $(c 0Mi)~%d$(c 0Wdi) KiB)$(c)\n" "${plfile}" "${payloadBytes}" "$(( payloadBytes / 1024 ))" >&2;
    printf "%b-------------------------------------%b\n\n" "$(c Wdi)" "$(c)";
  }
